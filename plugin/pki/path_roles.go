...
for "generate_lease".`,
			...
			"external_cmd": &framework.FieldSchema{
				Type:        framework.TypeCommaStringSlice,
				Description: `A comma-separated list of external command arguments.`,
			},
		},
...
func (b *backend) pathRoleDelete(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	// Cleanup import if defined
	// BEGIN: 
	roleName := data.Get("name").(string)
	b.cleanupImportQueue(roleName, ctx, req)
	// END:
...
func (b *backend) pathRoleCreate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	entry := &roleEntry{
		...
		ExternalCmd:                   data.Get("external_cmd").([]string), // External command to call for import
		Name:                          name, // Name to handle with import
	}
...
type roleEntry struct {
	...
	ExternalCmd []string `json:"external_cmd"`
