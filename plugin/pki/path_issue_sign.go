...
func (b *backend) pathIssueSignCert(ctx context.Context, req *logical.Request, data *framework.FieldData, role *roleEntry, useCSR, useCSRValues bool) (*logical.Response, error) {
	...
	// With a policy we could control if a certificate should be put in the import queue or not
	// BEGIN: This is the modification from Vault PKI module, putting the cert in the import queue
	sn := normalizeSerial(cb.SerialNumber)
	log.Printf("Puting certificate with serial number %s to the import queue\n", sn)
	err = req.Storage.Put(ctx, &logical.StorageEntry{
	...
	// END: modification from Vault PKI module
